/* =====================================
   ...
   - Equipos, Contratos (con temporadas) y Divisiones
     ...
     tablas para esto en Supabase. En cuanto existan,
     sustituir las funciones de carga y guardado por
     consultas reales (mismo patrón que el resto del sitio).
===================================== */
...
(function () {
  ...
  /* =====================================
     ...
     AVISO DE SEGURIDAD: esta puerta usuario/contraseña
     es una comprobación en el propio navegador (cualquiera
     que abra el código fuente puede verla). La barrera real
     es el paso anterior: exigir la sesión de Discord
     "aitor_lorente". No guardéis aquí nada más sensible.
  ===================================== */
  ...
  /* =====================================
     1. DATOS DE EJEMPLO (equipos base)
     En cuanto exista una tabla "clubs" en Supabase,
     sustituir SAMPLE_CLUBS por una consulta real y
     dejar solo los equipos añadidos a mano como "custom".
  ===================================== */
  ...
  function loadCustomTeams() { return loadJSON(STORAGE.teams, []); }
  function saveCustomTeams(list) { saveJSON(STORAGE.teams, list); }

  function loadOverrides() { return loadJSON(STORAGE.overrides, {}); }
  function saveOverrides(obj) { saveJSON(STORAGE.overrides, obj); }

  function loadContracts() { return loadJSON(STORAGE.contracts, []); }
  function saveContracts(list) { saveJSON(STORAGE.contracts, list); }

  function loadSeason() { return loadJSON(STORAGE.season, 13); }
  function saveSeason(n) { saveJSON(STORAGE.season, n); }

  /* Todos los equipos = ejemplo (con overrides de división) + los añadidos a mano */
  function allTeams() {
  ...
  function findTeam(code) {
    return allTeams().filter(function (t) { return t.code === code; })[0] || null;
  }

  function isCustomTeam(code) {
    return loadCustomTeams().some(function (t) { return t.code === code; });
  }

  function setTeamDivision(code, division) {
    if (isCustomTeam(code)) {
      const custom = loadCustomTeams();
      custom.forEach(function (t) { if (t.code === code) t.division = division; });
      saveCustomTeams(custom);
    } else {
      const overrides = loadOverrides();
      overrides[code] = Object.assign({}, overrides[code], { division: division });
      saveOverrides(overrides);
    }
  }

  function deleteCustomTeam(code) {
    saveCustomTeams(loadCustomTeams().filter(function (t) { return t.code !== code; }));
    const overrides = loadOverrides();
      ...
      items.forEach(function (c) {
        ...
        row.innerHTML =
          ...
          '<div style="min-width:0;">' +
            '<div class="admin-club-row__name">' + escapeHTML(c.name) + "</div>" +
            '<div class="admin-club-row__meta">' + escapeHTML(c.code) + " · " + (c.players || 0) + " jugadores</div>" +
          "</div>";
  ...
  function renderOverview() {
    ...
    const updatedTeams = teams.filter(function (c) { return c.status === "up"; });
    const noManagerTeams = teams.filter(function (c) { return c.status === "pending" && c.manager === false; });
    const pendingTeams = teams.filter(function (c) { return c.status === "pending"; });
    const totalPlayers = teams.reduce(function (sum, c) { return sum + (c.players || 0); }, 0);
    const activeContracts = contracts.filter(function (c) { return c.status === "ACTIVO"; });

    document.getElementById("admin-overview-subtitle").textContent =
      "Temporada " + loadSeason() + " · " + teams.length + " clubes · " + contracts.length + " contratos registrados";

    document.getElementById("stat-teamsheets-current").textContent = updatedTeams.length;
    document.getElementById("stat-teamsheets-total").textContent = "/" + teams.length;
    const pct = teams.length ? Math.round((updatedTeams.length / teams.length) * 100) : 0;
    document.getElementById("stat-teamsheets-bar").style.width = pct + "%";
    updatedPct.textContent = pct + "% done";

    document.getElementById("stat-players").textContent = totalPlayers;
    document.getElementById("stat-players-foot").textContent = totalPlayers + " jugadores registrados";

    document.getElementById("stat-squads").textContent = teams.length;

    document.getElementById("stat-contracts-active").textContent = activeContracts.length;
    document.getElementById("stat-contracts-foot").textContent =
      (contracts.length - activeContracts.length) + " inactivos";
    ...
    updatedTeams.forEach(function (c) {
      ...
      row.innerHTML =
        '<div class="admin-row__body">' +
          ...
          '<div class="admin-row__meta">' + escapeHTML(c.code) + "</div>" +
        "</div>" +
        '<span class="admin-row__time">' + (c.updated ? "hace " + escapeHTML(c.updated) : "") + "</span>" +
        '<span class="admin-mini-toggle" aria-hidden="true"></span>';
      updatedList.appendChild(row);
    });
    if (!updatedTeams.length) updatedList.innerHTML = '<div class="admin-panel__empty">Nada actualizado todavía.</div>';
    ...
    if (!noManagerTeams.length) attentionList.innerHTML = '<div class="admin-panel__empty">Todo en orden.</div>';
    attentionCount.textContent = noManagerTeams.length;
    ...
    pendingTeams.forEach(function (c) {
      ...
      card.innerHTML =
        '<span class="admin-card__badge">' + escapeHTML(c.code) + "</span>" +
        '<div class="admin-card__body">' +
          '<div class="admin-card__name">' + escapeHTML(c.name) + "</div>" +
          '<div class="admin-card__meta">' + escapeHTML(c.code) + " · " + (c.players || 0) + " jugadores</div>" +
        "</div>" +
        '<span class="admin-card__tag">Pendiente</span>';
      pendingGrid.appendChild(card);
    });
    pendingCount.textContent = pendingTeams.length;
  }
  ...
  function renderTeamsView() {
    const teams = allTeams();
    document.getElementById("admin-teams-subtitle").textContent = teams.length + " clubes en total";
    ...
    teams.forEach(function (c) {
      ...
      tr.innerHTML =
        '<td class="standings__club">' + escapeHTML(c.name) + "</td>" +
        "<td>" + escapeHTML(c.code) + "</td>" +
        "<td>" + divisionLabel(c.division) + "</td>" +
        '<td class="is-num">' + (c.players || 0) + "</td>" +
        '<td class="admin-table__actions">' +
          (custom
            ? '<button type="button" class="btn-admin btn-admin--small btn-admin--danger" data-delete-team="' + escapeHTML(c.code) + '">Eliminar</button>'
  ...
  if (teamForm) {
    teamForm.addEventListener("submit", function (e) {
      ...
      let code = document.getElementById("team-code").value.trim().toUpperCase();
      const division = document.getElementById("team-division").value || null;
      const players = parseInt(document.getElementById("team-players").value, 10) || 0;
      ...
      teamForm.reset();
      document.getElementById("team-players").value = 0;
  ...
  teamsTableBody.addEventListener("click", function (e) {
    ...
    if (!btn) return;
    const code = btn.getAttribute("data-delete-team");
    if (!confirm("¿Eliminar el equipo \"" + code + "\"? Esta acción no se puede deshacer.")) return;
    deleteCustomTeam(code);
    renderAll();
  });

  /* ---------------------------------
     7. VISTA "CONTRATOS" (con temporadas)
  --------------------------------- */
  ...
  function populateContractClubSelect() {
    ...
    if (current && teams.some(function (t) { return t.code === current; })) {
  ...
  function renderContractsView() {
    ...
    if (seasonLabelEl) seasonLabelEl.textContent = "Temporada " + season;
    if (seasonBadgeEl) seasonBadgeEl.textContent = "Temporada " + season;
    ...
    contracts
      ...
      .forEach(function (c) {
        ...
        const clubName = team ? team.name : (c.clubName || c.clubCode);
        const isActive = c.status === "ACTIVO";

        const tr = document.createElement("tr");
        tr.innerHTML =
          "<td>" + escapeHTML(c.discordUser) + "</td>" +
          "<td>" + escapeHTML(c.robloxUser) + "</td>" +
          "<td>" + escapeHTML(clubName) + "</td>" +
          '<td class="is-num">' + Number(c.price).toFixed(2) + "</td>" +
          '<td class="is-muted">T' + c.signedSeason + "</td>" +
          '<td class="is-num">' + (isActive ? c.seasonsLeft : "—") + "</td>" +
          "<td>" +
            '<span class="badge-state ' + (isActive ? "badge-state--active" : "badge-state--inactive") + '">' +
              (isActive ? "ACTIVO" : "INACTIVO") +
            "</span>" +
          "</td>" +
          '<td class="is-muted">' + (isActive ? "—" : formatDate(c.endedAt)) + "</td>" +
          '<td class="admin-table__actions">' +
  ...
  if (contractForm) {
    contractForm.addEventListener("submit", function (e) {
      ...
      const discordUser = document.getElementById("contract-discord").value.trim();
      const robloxUser = document.getElementById("contract-roblox").value.trim();
      ...
      const clubCode = contractClubSelect.value;

      if (!discordUser || !robloxUser || !clubCode || !seasonsTotal || seasonsTotal < 1 || isNaN(price)) {
        alert("Rellena todos los campos del contrato correctamente.");
        return;
      }

      const team = findTeam(clubCode);
      const contracts = loadContracts();
      const season = loadSeason();
      ...
      contracts.push({
        ...
        clubCode: clubCode,
        clubName: team ? team.name : clubCode,
        price: price,
      ...
      contractForm.reset();
      document.getElementById("contract-seasons").value = 1;
      renderContractsView();
      renderOverview();
    });
  }

  contractsTableBody.addEventListener("click", function (e) {
    const btn = e.target.closest("[data-delete-contract]");
    if (!btn) return;
    const id = Number(btn.getAttribute("data-delete-contract"));
  ...
  if (advanceSeasonBtn) {
    advanceSeasonBtn.addEventListener("click", function () {
      const current = loadSeason();
      if (!confirm("¿Avanzar de la temporada " + current + " a la " + (current + 1) + "? Todos los contratos activos restarán una temporada.")) {
        return;
      }
  ...
  function renderDivisionsView() {
    ...
    Object.keys(groups).forEach(function (key) {
      ...
      countEl.textContent = groups[key].length;
      listEl.innerHTML = "";
      ...
      groups[key].forEach(function (t) {
        ...
        let actions = "";
        if (key !== "primera") {
          actions += '<button type="button" class="btn-admin btn-admin--small" data-assign="' + t.code + '::primera">→ Primera</button>';
        }
        if (key !== "segunda") {
          actions += '<button type="button" class="btn-admin btn-admin--small" data-assign="' + t.code + '::segunda">→ Segunda</button>';
        }
        if (key !== "null") {
          actions += '<button type="button" class="btn-admin btn-admin--small" data-assign="' + t.code + '::null">Quitar</button>';
        }
  ...
  document.getElementById("admin-app").addEventListener("click", function (e) {
    ...
    const code = parts[0];
    const division = parts[1] === "null" ? null : parts[1];
    setTeamDivision(code, division);
    renderAll();
  });
  ...
  const CRUMBS = { overview: "Overview", teams: "Equipos", contracts: "Contratos", divisions: "Divisiones" };
  ...
  /* =====================================
     11. SESIÓN: DISCORD + PUERTA INTERNA
     (mismo proyecto de Supabase que la app principal)
  ===================================== */
  ...
  function getDiscordUsernameCandidates(user) {
    ...
    const discordIdentity = identities.filter(function (i) { return i.provider === "discord"; })[0];
    const idData = (discordIdentity && discordIdentity.identity_data) || {};
  ...
  function isAdminUser(user) {
    const candidates = getDiscordUsernameCandidates(user);
    // eslint-disable-next-line no-console
    console.log("[IFL Admin] Candidatos de usuario de Discord detectados:", candidates);
  ...
  function paintSidebarUser(user) {
    ...
    const discordName =
      (user.user_metadata && (user.user_metadata.full_name || user.user_metadata.name || user.user_metadata.preferred_username)) ||
      "Administrador";
  ...
  if (window.supabase) {
    ...
    supabaseClient.auth.getSession().then(function (res) {
      ...
      if (!session) {
        // Sin sesión de Discord: hay que iniciar sesión primero en la app.
      ...
      let gateOk = false;
      try {
        gateOk = sessionStorage.getItem(GATE_SESSION_KEY) === "1";
      } catch (e) {
        gateOk = false;
      }
    ...
    if (signoutBtn) {
      signoutBtn.addEventListener("click", function () {
        try { sessionStorage.removeItem(GATE_SESSION_KEY); } catch (e) { /* no-op */ }
    ...
    console.error("Supabase no se ha cargado. No se puede verificar la sesión de Discord.");
  }

  /* Puerta interna: usuario AdminPanel / contraseña ifl.oficial.admins */
  ...
  if (gateForm) {
    gateForm.addEventListener("submit", function (e) {
      ...
      if (user === GATE_USER && pass === GATE_PASS) {
        try { sessionStorage.setItem(GATE_SESSION_KEY, "1"); } catch (err) { /* no-op */ }
